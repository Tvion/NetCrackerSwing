...
public class AddDialog extends JDialog {
    ...
    public static boolean verifyValues(JDialog dialog,String[] values){
        ...
        if(!m.matches()) { JOptionPane.showMessageDialog(dialog,"Name must contains symbols A-z"); return false;}
        m=names.matcher(values[3]);
        if(!m.matches()) { JOptionPane.showMessageDialog(dialog,"Authors name must contains 3-15 symbols A-z"); return false;}
        m=num.matcher(values[1]);
        if(!m.matches()) { JOptionPane.showMessageDialog(dialog,"Count must be 1-10000000"); return false;}
        m=price.matcher(values[2]);
        if(!m.matches()) { JOptionPane.showMessageDialog(dialog,"Price must be Double"); return false;}
        m=gender.matcher(values[5]);
        if(!m.matches()) { JOptionPane.showMessageDialog(dialog,"Gender must be f or m"); return false;}
        m=email.matcher(values[4]);
        if(!m.matches()) { JOptionPane.showMessageDialog(dialog,"Wrong Email Format"); return false;}
        return true;
    }
